...
export default function StoragePage() {
  ...
  // KV state
  const [kvKey, setKvKey] = useState("hedera-sdk:token-transfer-bug");
    ...
    >
      ...
      <section style={{ margin: "24px 0" }}>
        ...
        <p style={{ color: "#888", fontSize: 13 }}>
          ...
          immutable and content-addressed. The root hash can be logged to Hedera
          HCS for cross-chain verification.
        </p>
        <div>
          <label>
            Key:{" "}
  ...
  Log to Hedera HCS: {item_id, version: 1, content_hash: "0xabc..."}

Knowledge item updated (v2):
  Upload new version → 0G Storage → root hash: 0xdef...
  Log to Hedera HCS: {item_id, version: 2, content_hash: "0xdef...", prev: "0xabc..."}

Verification:
  1. Download from 0G Storage (by root hash)
  2. Hash it locally
  3. Compare to Hedera HCS record
  4. If match → content is authentic and untampered
